...
from robopilot.parts.kinematics import NormalizeSteeringAngle, UnnormalizeSteeringAngle, TwoWheelSteeringThrottle
from robopilot.parts.kinematics import Unicycle, InverseUnicycle, UnicycleUnnormalizeAngularVelocity
from robopilot.parts.kinematics import Bicycle, InverseBicycle, BicycleUnnormalizeAngularVelocity
from robopilot.parts.explode import ExplodeDict
from robopilot.parts.transform import Lambda
from robopilot.parts.pipe import Pipe
...
class ToggleRecording:
    def __init__(self, auto_record_on_throttle, record_in_autopilot):
        """
        Robopilot Part that manages the recording state.
        """
    ...
    if use_joystick or cfg.USE_JOYSTICK_AS_DEFAULT:
        ...
        if cfg.CONTROLLER_TYPE == "pigpio_rc":  # an RC controllers read by GPIO pins. They typically don't have buttons
            ...
            if cfg.CONTROLLER_TYPE == "custom":  # custom controller created with `robopilot createjs` command
                ...
                from robopilot.parts.controller import MockController
...
def add_camera(V, cfg, camera_type):
    ...
    if camera_type == "stereo":
        ...
        V.add(StereoPair(), inputs=['cam/image_array_a', 'cam/image_array_b'],
            outputs=['cam/image_array'])
        if cfg.BGR2RGB:
            from robopilot.parts.cv import ImgBGR2RGB
...
def add_odometry(V, cfg, threaded=True):
    ...
    :param cfg: the configuration (from myconfig.py)
    """
    from robopilot.parts.pose import BicyclePose, UnicyclePose
...
def add_drivetrain(V, cfg):
    ...
    if (not cfg.DONKEY_GYM) and cfg.DRIVE_TRAIN_TYPE != "MOCK":
        from robopilot.parts import actuator, pins
        from robopilot.parts.actuator import TwoWheelSteeringThrottle
        ...
        if cfg.DRIVE_TRAIN_TYPE == "PWM_STEERING_THROTTLE":
            ...
            #
            from robopilot.parts.actuator import PWMSteering, PWMThrottle, PulseController
            ...
            from robopilot.parts.actuator import VESC
