#!/usr/bin/env python3
"""
Scripts to record a path by driving a robopilot car
...
from robopilot.parts.controller import JoystickController
from robopilot.parts.path import CsvThrottlePath, PathPlot, CTE, PID_Pilot, \
    PlotCircle, PImage, OriginOffset
from robopilot.parts.transform import PIDController
from robopilot.parts.kinematics import TwoWheelSteeringThrottle
from robopilot.templates.complete import add_odometry, add_camera, \
    add_user_controller, add_drivetrain, add_simulator, add_imu, DriveMode, \
    UserPilotCondition, ToggleRecording
from robopilot.parts.logger import LoggerPart
...
def drive(cfg, use_joystick=False, camera_type='single'):
    ...
    if cfg.HAVE_SOMBRERO:
        from robopilot.utils import Sombrero
        s = Sombrero()
    ...
    if cfg.HAVE_T265:
        from robopilot.parts.realsense2 import RS_T265
        if cfg.HAVE_ODOM and not os.path.exists(cfg.WHEEL_ODOM_CALIB):
            ...
            print("cp robopilot/robopilot/templates/calibration_odometry.json .")
            exit(1)
...
def add_gps(V, cfg):
    if cfg.HAVE_GPS:
        from robopilot.parts.serial_port import SerialPort, SerialLineReader
        from robopilot.parts.gps import GpsNmeaPositions, GpsLatestPosition, GpsPlayer
        from robopilot.parts.pipe import Pipe
        from robopilot.parts.text_writer import CsvLogger
