...
def test_tubplot(cardir):
    ...
    from robopilot.parts.keras import KerasLinear
    ...
    with open(cfg_file, "w+") as f:
        f.writelines(["# config file\n", "IMAGE_H = 120\n", "IMAGE_W = 160\n",
                      "IMAGE_DEPTH = 3\n", "\n"])
    cmd = ['robopilot', 'tubplot', '--tub', tub_dir, '--model', model_path,
           '--type', 'linear', '--noshow']

    # run robopilot command in subprocess
