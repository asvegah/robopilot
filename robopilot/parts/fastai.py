...
import torch
from robopilot.utils import normalize_image, linear_bin
from robopilot.pipeline.types import TubRecord, TubDataset
from robopilot.pipeline.sequence import TubSequence
from robopilot.parts.interpreter import FastAIInterpreter, Interpreter, KerasInterpreter
from robopilot.parts.pytorch.torch_data import TorchTubDataset, get_default_transform
...
class FastAiPilot(ABC):
    ...
    def run(self, img_arr: np.ndarray, other_arr: List[float] = None) \
        ...
        """
        Robopilot parts interface to run the part in the loop.
