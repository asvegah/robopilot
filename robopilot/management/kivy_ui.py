...
from robopilot import load_config
from robopilot.parts.keras import KerasMemory
...
from robopilot.pipeline.types import TubRecord
from robopilot.utils import get_model_by_type
from robopilot.pipeline.training import train
...
class RcFileHandler:
    ...
    def __init__(self, file_path='~/.robopilotrc'):
    ...
    def read_file(self):
        if os.path.exists(self.file_path):
            with open(self.file_path) as f:
                ...
                Logger.info(f'Robopilotrc: Robopilot file {self.file_path} loaded.')
                return data
        else:
            Logger.warn(f'Robopilotrc: Robopilot file {self.file_path} does not '
                        f'exist.')
            return {}

    def write_file(self):
        if os.path.exists(self.file_path):
            Logger.info(f'Robopilotrc: Robopilot file {self.file_path} updated.')
...
class ControlPanel(BoxLayout):
    ...
    def step(self, fwd=True, continuous=False, *largs):
        ...
        msg = f'Robopilot {"run" if continuous else "step"} ' \
    ...
    def on_keyboard(self, key, scancode):
        ...
        if key == ' ':
            if self.clock and self.clock.is_triggered:
                ...
                self.screen.status('Robopilot stopped')
            else:
...
class OverlayImage(FullImage):
    ...
    def get_image(self, record):
        from robopilot.management.makemovie import MakeMovie
...
class StartScreen(Screen):
    img_path = os.path.realpath(os.path.join(
        ...
        '../parts/web_controller/templates/static/robopilot-logo-sideways.png'))
    pass


class RobopilotApp(App):
    start_screen = None
    tub_screen = None
    train_screen = None
    pilot_screen = None
    car_screen = None
    title = 'Robopilot Manager'
    ...
    def build(self):
        self.start_screen = StartScreen(name='robopilot')
